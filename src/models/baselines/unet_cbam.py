...
@register_model("unet_cbam")
class UNetCBAM(nn.Module):
    """Fair control baseline: plain U-Net with CBAM only in the decoder.

    This isolates the effect of decoder attention from the Hartley-Fourier block.
    """
    ...
    def __init__(
        ...
        self.decoder = UNetDecoder(channels=channels, norm=norm, act=act, use_cbam=True)
