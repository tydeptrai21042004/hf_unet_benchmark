...
class HSNet(nn.Module):
    """Adapted HSNet baseline with CSA, HSC, and MSP modules.

    This keeps the paper-defining module structure while using local backbones that fit
    the benchmark framework and resource budget.
    """
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.faithful_output:
            return {
                "main": fused,
                "aux": [p4, p3, p2, p1],
                "msp_weights": weights.detach(),
                "stage_logits": [p4, p3, p2, p1, p0],
            }
        return fused
